...
export const useWorkflowManagement = (canvas: Ref<HTMLElement | undefined>, zoom: Ref<number>) => {
  ...
  const configureBlock = async (blockId: string, onConfigurationChanged?: (config: ServiceConfiguration) => void) => {
    ...
    const resolvedService = resolveService(serviceName)
    if (!resolvedService) {
  ...
  const mapBackendNodeToBlock = (node: BackendWorkflowNode): WorkflowBlockData | null => {
    ...
    if (node.serviceName) {
      ...
      console.warn(`[WorkflowManagement] Node has serviceId but no serviceName:`, node.serviceId)
      return null
    }
  ...
  const updateBlockConfig = async (blockId: string, newConfig: ServiceConfiguration) => {
    console.log('[WorkflowManagement] Updating block config:', blockId, newConfig)
    
    const blockIndex = workflowBlocks.value.findIndex(b => b.id === blockId)
    if (blockIndex === -1) {
      console.error('[WorkflowManagement] Block not found:', blockId)
      return
    }

    const block = workflowBlocks.value[blockIndex]
    if (!block) {
      console.error('[WorkflowManagement] Block is undefined:', blockId)
      return
    }
    ...
    if (currentAreaId.value && block.id.startsWith('node-')) {
      try {
        ...
        console.log('[WorkflowManagement] Block config updated on backend')
      } catch (err: any) {
        saveError.value = err.message || 'Erreur lors de la mise à jour'
        console.error('[WorkflowManagement] Error updating block config:', err)
        throw err
      } finally {
        isSaving.value = false
      }
    } else {
      console.log('[WorkflowManagement] Block config updated locally (not yet saved to backend)')
    }
  }
