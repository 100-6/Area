...
class AuthService {
    ...
    /**
     * Obtenir l'URL d'authentification Notion
     */
    getNotionAuthUrl(state?: string): string {
        if (!this.oauthManager.isNotionConfigured())
            throw new Error('NOTION_OAUTH_NOT_CONFIGURED');
        return this.oauthManager.getNotionAuthUrl(state);
    }
    ...
    async handleTrelloCallback(token: string, authenticatedUserId?: string): Promise<AuthResult> {
        try {
            ...
            throw new Error('TRELLO_OAUTH_ERROR');
        }
    }

    /**
     * Gérer le callback Notion OAuth
     * @param code - Authorization code from Notion
     * @param authenticatedUserId - Optional user ID if already authenticated (for linking accounts)
     */
    async handleNotionCallback(code: string, authenticatedUserId?: string): Promise<AuthResult> {
        try {
            const user = await this.oauthManager.handleNotionCallback(code, authenticatedUserId);
            ...
            const refreshExpiry = this.jwtManager.getTokenExpiry(refreshToken) || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
            await UserSession.create(user.id, refreshToken, refreshExpiry);
            console.log(`SUCCESS: Notion OAuth login: ${user.email} (ID: ${user.id})`.green);
            return {
                user: {
                    id: user.id,
                    email: user.email,
    ...
    getSpotifyAuthUrl(state?: string): string {
        if (!this.oauthManager.isSpotifyConfigured())
            throw new Error('SPOTIFY_OAUTH_NOT_CONFIGURED');
        return this.oauthManager.getSpotifyAuthUrl(state);
    }

    /**
     * Gérer le callback Spotify OAuth
     * @param code - OAuth authorization code
     * @param authenticatedUserId - Optional user ID if already authenticated (for linking accounts)
     */
    async handleSpotifyCallback(code: string, authenticatedUserId?: string): Promise<AuthResult> {
        try {
            const user = await this.oauthManager.handleSpotifyCallback(code, authenticatedUserId);
            ...
            const refreshExpiry = this.jwtManager.getTokenExpiry(refreshToken) || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
            await UserSession.create(user.id, refreshToken, refreshExpiry);
            console.log(`SUCCESS: Spotify OAuth login: ${user.email} (ID: ${user.id})`.green);
            return {
                user: {
                    id: user.id,
                    email: user.email,
    ...
    /**
     * Vérifier si Spotify OAuth est configuré
     */
    isSpotifyConfigured(): boolean {
        return this.oauthManager.isSpotifyConfigured();
    }

    /**
     * Obtenir l'URL d'authentification Twitch
     */
    getTwitchAuthUrl(state?: string): string {
        if (!this.oauthManager.isTwitchConfigured())
            throw new Error('TWITCH_OAUTH_NOT_CONFIGURED');
        return this.oauthManager.getTwitchAuthUrl(state);
    }

    /**
     * Gérer le callback Twitch OAuth
     * @param code - OAuth authorization code
     * @param authenticatedUserId - Optional user ID if already authenticated (for linking accounts)
     */
    async handleTwitchCallback(code: string, authenticatedUserId?: string): Promise<AuthResult> {
        try {
            const user = await this.oauthManager.handleTwitchCallback(code, authenticatedUserId);
            ...
            const refreshExpiry = this.jwtManager.getTokenExpiry(refreshToken) || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
            await UserSession.create(user.id, refreshToken, refreshExpiry);
            console.log(`SUCCESS: Twitch OAuth login: ${user.email} (ID: ${user.id})`.green);
            return {
                user: {
                    id: user.id,
                    email: user.email,
    ...
    /**
     * Vérifier si Twitch OAuth est configuré
     */
    isTwitchConfigured(): boolean {
        return this.oauthManager.isTwitchConfigured();
    }
}
