 ...
 */
export class SendNotification extends BaseAction {
    getName(): string {
        return 'send_notification';
    }

    getDescription(): string {
        return 'Send a notification to a ntfy topic';
    }
    ...
    validate(config: ActionConfig): boolean {
        ...
        if (config.delay) {
            const delayRegex = /^\d+(s|m|min|h|d)$/;
            if (!delayRegex.test(config.delay)) {
                throw new Error('delay must be in format: number + unit (e.g., "30min", "2h", "1d")');
            }
        }
    ...
    async execute(config: ActionConfig, context: ActionContext): Promise<ActionResult> {
        ...
        try {
            console.log(`[SendNotification] Executing for AREA ${context.areaId}`.cyan);
            ...
            const delay = config.delay ? this.replaceVariables(config.delay, context) : undefined;

            const url = `${baseUrl}/${topic}`;

            console.log(`[SendNotification] Sending notification to topic "${topic}"`.cyan);
            console.log(`[SendNotification] Message: "${message.substring(0, 50)}${message.length > 50 ? '...' : ''}"`.cyan);
            ...
            console.log(`[SendNotification] ✓ Notification sent successfully`.green);
            console.log(`[SendNotification] Response status: ${response.status}`.green);
            ...
            const executionTime = Date.now() - startTime;
            console.error(`[SendNotification] ❌ Failed to send notification:`.red, error);

            let errorMessage = 'Unknown error';
            if (axios.isAxiosError(error)) {
                errorMessage = error.response?.data?.error || error.message;
                console.error(`[SendNotification] Status: ${error.response?.status}`.red);
